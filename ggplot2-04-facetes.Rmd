...
```{r}
#Nėra prasminga braižyti duomenų
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy)) + 
  facet_grid(cty ~ cyl)
```
...
```{r}
# Nėra tokių reikšmių, kurios turėtų duomenis iš abiejų kintamųjų.
ggplot(data = mpg) + 
...
```{r}
# Yra tik 1 stulpelis ir eilutėse yra drv kintamojo reikšmės
ggplot(data = mpg) + 
    geom_point(mapping = aes(x = displ, y = hwy)) + 
    facet_grid(drv ~ .) 
